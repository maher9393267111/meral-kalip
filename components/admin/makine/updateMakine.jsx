...
import ProductForm from "./makineForm";
...
const UpdateMakineMain = ({ product, cats, subcats }) => {
  const { query, replace } = useRouter();
  const { id } = query;
  console.log("IDDD___--->>" , id  ,query)
  ...
  const onFinish = async (values) => {
    try {
      ...
      if (!file && !values?.image) {
        ...
        values.image = await uploadImages(file, true, "makine");
  ...
  if (file || values?.image){
    ...
    console.log("INITI IMAGE --->" , values?.image)
      ...
      const newImagesUploaded =  await uploadImages(files, false, "makines");
      console.log("UPLOADED--->" ,newImagesUploaded)
      values.images = [...values.images, ...newImagesUploaded];
     await updateDoc(doc(db, "makines", id), values);
...
export default UpdateMakineMain;
